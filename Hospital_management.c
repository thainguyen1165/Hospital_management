...
void print_saved_output_message(const char* output_file);
...
int priority_compare(struct PatientInQueue* a, struct PatientInQueue* b){
    ...
    if (a->medical_priority != b->medical_priority){
        return a->medical_priority - b->medical_priority;
    }
...
void register_for_examination(struct MedicalSystemState* system, struct Patient* patient_ptr, const char* dept_id, double spo2, int pulse, int bp, double temp){
    ...
    entry->queue_priority_score = medical_priority * 1000;
    if (patient_ptr->is_priority_object){
        entry->queue_priority_score += 100;
    }
...
void update_priority_for_long_waiters(struct Department* dept, long current_time){
    ...
    for (i = 0; i < dept->wait_queue.size; i++){
        waited_seconds = current_time - dept->wait_queue.heap_array[i]->registration_time;
        if (waited_seconds < 0){
            waited_seconds = 0;
        }
        bonus = (int)(waited_seconds / 900L) * 50;
        dept->wait_queue.heap_array[i]->queue_priority_score =
            dept->wait_queue.heap_array[i]->medical_priority * 1000 + bonus;
        if (dept->wait_queue.heap_array[i]->root_record->is_priority_object){
            dept->wait_queue.heap_array[i]->queue_priority_score += 100;
        }
        if (dept->wait_queue.heap_array[i]->absent_count > 0) {
            dept->wait_queue.heap_array[i]->queue_priority_score -= dept->wait_queue.heap_array[i]->absent_count * 200;
        }
    }
...
void handle_absent_patient(struct Department* dept, struct PatientInQueue* entry){
    ...
    entry->queue_priority_score -= 200;
    if (entry->absent_count >= 2){
        printf("Benh nhan %s da vang mat 2 lan, benh nhan bi xoa khoi hang doi.\n", entry->root_record->patient_id);
        free(entry);
        return;
    }
    printf("Benh nhan %s duoc danh dau vang mat. Da dua tro lai hang doi voi diem xep hang thap hon.\n", entry->root_record->patient_id);
    entry->queue_status = 0;
    max_heap_insert(&dept->wait_queue, entry);
}
...
struct Patient* find_patient(struct MedicalSystemState* system, const char* patient_id){
    ...
    for (i = 0; i < system->patient_records_array.size; i++){
        p = (struct Patient*)system->patient_records_array.elements[i];
        if (strcmp(p->patient_id, patient_id) == 0){
            return p;
        }
    }
    return NULL;
...
struct Patient* create_patient(struct MedicalSystemState* system, const char* name, const char* birth, int gender, const char* citizen_id, const char* phone, const char* email, const char* address, const char* blood, int priority_object, const char* priority_type){
    ...
    sprintf(p->patient_id, "BN%06d", system->next_patient_id_counter);
...
void menu_add_patient(struct MedicalSystemState* system){
    ...
    read_line("Thu dien tu: ", email, sizeof(email));
...
void choose_input_file(char* dest, int dest_size, const char* argv0){
    ...
    if (!file_exists(dest)) {
        printf("Khong tim thay file input.txt. Hay tao file input.txt truoc khi chay chuong trinh.\n");
        exit(1);
    }
}
...
void choose_output_file(char* dest, int dest_size, const char* input_path){
    (void)input_path;
    safe_copy(dest, (size_t)dest_size, "output.txt");
}
...
static struct Patient* create_patient_from_file(struct MedicalSystemState* system, char fields[][TEXT_LARGE]){
    ...
    safe_copy(p->patient_id, sizeof(p->patient_id), fields[0]);
    ...
    while (fgets(line, sizeof(line), f) != NULL){
        ...
        if (strcmp(fields[0], "BO_DEM") == 0 && count >= 4){
            ...
            for (i = 0; i < n; i++){
                ...
                patient = find_patient(system, fields[1]);
